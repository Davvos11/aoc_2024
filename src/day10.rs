...
struct Map<'a> {
    grid: &'a[Vec<u32>],
    memo: HashMap<(usize, usize), HashSet<(usize, usize)>>,
...
impl<'a> Map<'a> {
    pub fn new(grid: &'a[Vec<u32>]) -> Self {
        Self {
            grid, memo: HashMap::new(), visited: HashSet::new(),
        }
    }
    ...
    pub fn get_trail_ends(&mut self, y: usize, x: usize, current_h: u32) -> HashSet<(usize, usize)> {
        ...
        for (d_y, d_x) in DIRS {
            if let (Some(y_new), Some(x_new)) = (y.checked_add_signed(d_y), x.checked_add_signed(d_x)) {
                ...
                if let Some(&h) = self.grid.get(y_new).and_then(|row| row.get(x_new)) {
                    if h == current_h + 1 {
                        // First check cache
                        if let Some(memo) = self.memo.get(&(y_new, x_new)) {
                            result.extend(memo);
                            continue;
                        }
        ...
        self.memo.insert((y, x), result.clone());
        result
    }
}
...
fn rev_dir(y: isize, x: isize) -> (isize, isize) {
    (-y, -x)
}